...
describe("CatSideBarSearchNames", () => {
  ...
  it("populates search input from store", async () => {
    const { userStore } = renderCatSideBarSearchNames();
    userStore.nameSearch = "Oreo";
  ...
  it("writes user input to store", async () => {
    ...
    expect(userStore.UPDATE_NAME_SEARCH).toHaveBeenCalledWith("O");
  });
  ...
  it("removes whitespace from user input", async () => {
    ...
    expect(userStore.UPDATE_NAME_SEARCH).toHaveBeenCalledWith("Oreo");
  });
});